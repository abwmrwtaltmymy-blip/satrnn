...
# ==========================================
# 1. قائمة اختيار طريقة إضافة الحساب
# ==========================================
@bot.on(events.CallbackQuery(data=b"add_account"))
async def add_account_choice(event):
    if not await is_authorized(event.sender_id): return
    buttons = [
        [Button.inline("🤖 استخراج API تلقائي", b"add_acc_auto")],
        [Button.inline("✍️ إدخال API يدوي", b"add_acc_manual")],
        [Button.inline("🔙 رجوع", b"back_start")]
    ]
    await event.edit("🛠 **اختر طريقة إضافة الحساب:**\n\n▫️ **استخراج تلقائي:** البوت سيجلب `API_ID` و `API_HASH` برمز موقع my.telegram.\n▫️ **إدخال يدوي:** ستقوم بإدخال المفاتيح بنفسك إذا كانت جاهزة لديك.", buttons=buttons)


# ==========================================
# 2. الوضع اليدوي لإضافة الحساب
# ==========================================
@bot.on(events.CallbackQuery(data=b"add_acc_manual"))
async def add_acc_manual_handler(event):
    if not await is_authorized(event.sender_id): return
    await event.delete()
    ...
    async with bot.conversation(event.chat_id) as conv:
        ...
        if phone_msg.text.strip().startswith('/'): return
        phone = "".join(c for c in phone_msg.text if c.isdigit() or c == '+')

        await conv.send_message("🔑 **أدخل API_ID الخاص بالحساب (أرقام فقط):**")
        try: api_id_msg = await conv.get_response(timeout=300)
        except asyncio.TimeoutError: return
        if api_id_msg.text.strip().startswith('/'): return
        user_api_id = int(api_id_msg.text.strip())

        await conv.send_message("🔐 **أدخل API_HASH الخاص بالحساب:**")
        try: api_hash_msg = await conv.get_response(timeout=300)
        except asyncio.TimeoutError: return
        if api_hash_msg.text.strip().startswith('/'): return
        ...
        async with managed_client(session_path, user_api_id, user_api_hash) as user_client:
            ...
            await conv.send_message("📩 **تم إرسال كود التحقق. للحماية من الحظر، يجب إرسال الكود مع مسافات بين الأرقام (مثال: 1 2 3 4 5):**")
            try: code_msg = await conv.get_response(timeout=300)
            except asyncio.TimeoutError: return
            
            # --- حماية المسافات الصارمة ---
            if " " not in code_msg.text.strip():
                await conv.send_message("❌ **مرفوض!** لم تقم بوضع مسافات بين الأرقام (مثل: 0 0 0 0 0). هذا الإجراء إجباري للحماية من الحظر. يرجى إعادة الإضافة من البداية.", buttons=[[Button.inline("🔙 رجوع", b"back_start")]])
                return
...
# ==========================================
# 3. الوضع التلقائي (نفس القديم مع إضافة الحماية الصارمة)
# ==========================================
@bot.on(events.CallbackQuery(data=b"add_acc_auto"))
async def add_acc_auto_handler(event):
    if not await is_authorized(event.sender_id): return
    await event.delete()
    ...
    async with bot.conversation(event.chat_id) as conv:
        ...
        try: web_code_msg = await conv.get_response(timeout=300)
        except asyncio.TimeoutError:
            await extractor.close()
            return
        ...
        async with managed_client(session_path, user_api_id, user_api_hash) as user_client:
            ...
            try: await user_client.sign_in(phone=phone, code=code, phone_code_hash=send_code.phone_code_hash)
            except errors.SessionPasswordNeededError:
                await conv.send_message("🔒 **أدخل كلمة المرور (التحقق بخطوتين):**")
                try: pass_msg = await conv.get_response(timeout=300)
                except asyncio.TimeoutError: return
